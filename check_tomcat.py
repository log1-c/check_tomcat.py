...
def define_range(str):
    if is_float_try(str):
        ...
        message = f"bad range definition in {str}"
        print(message)
        sys.exit(1)  # Error in range definition
    if range[0] < range[1]:
        return range  # OK
    else:
        message = f"Second value of range {str} is less than first value"
        print(message)
        exit(1)
...
def define_status(value, warning, critical):
    ...
    if args.verbosity:
        print(f"Value for test: {str(value)}")
        print(
            f"Warning range (min:{str(warning_range[0])} max:{str(warning_range[1])} in_range:{str(warning_range[2])})")
        print(
            f"Critical range (min:{str(critical_range[0])} max:{str(critical_range[1])} in_range:{str(critical_range[2])})")
        print("")

    # value into the range range(x:y:True)
    if warning_range[2] is True and critical_range[2] is True:
    ...
    elif warning_range[2] is False and critical_range[2] is False:
        if (warning_range[1] < critical_range[1]) or (warning_range[0] > critical_range[0]):
            parser.print_usage()
            parser.exit(3, "ERRROR: critical range (%s) is greater than warning range(%s)\n" % (critical, warning))
...
def read_page(host, port, url, user, password):
    ...
    if args.verbosity:
        print(f"connection url: {url_tomcat}\n")
    ...
    try:
        ...
        req = urllib.request.Request(url_tomcat)
        handle = urllib.request.urlopen(req, None)
    ...
    if args.verbosity > 2:
        print(f"page {url_tomcat} content:")
        print(page)
        print("")
    return page, error
...
if (error_serverinfo == False):
    ...
    if args.verbosity > 2:
        print(f"tomcat_version_string: {tomcat_version_string}")
        print(f"tomcat_status_string: {tomcat_status_string}")
    ...
    if args.verbosity:
        print(f"tomcat_version: {tomcat_version}")
    # If i can't read the tomcat version because it is not a number
    if not tomcat_version.isdigit():
        tomcat_version = 0
        if args.verbosity:
...
if args.mode == 'status':
    ...
    if not error_serverinfo:
        ...
        if (tomcat_status_string.find("OK") != -1):
            output = f"{tomcat_version_string} server is OK"
            exit_status = 'OK'
        else:
            output = f"This server is not a tomcat server or {url_serverinfo} is not the manager app server info page"
            exit_status = 'UNKNOWN'
    # if serverinfo page is not correct try with th status xml page
    else:
        tree_xml, error_status_xml = read_page_status_XML(args.host, args.port, args.URL, args.user,
                                                          args.authentication)
        # check if page status xml is OK
        if not error_status_xml:
    ...
    if error_status_xml:
        ...
        if tree_xml is not None:
            ...
            if args.verbosity:
                ...
                if args.verbosity > 1:
                    print((
                        "free:%0.1f total:%0.1f max:%0.1f available:%0.1f used:%0.1f percent_used:%0.2f" % (free_memory,
                                                                                                            total_memory,
                                                                                                            max_memory,
                                                                                                            available_memory,
                                                                                                            used_memory,
                                                                                                            percent_used_memory)))
                print(("free_memory:%s total memory:%s max_memory:%s" % (
                    sizeof_fmt(free_memory), sizeof_fmt(total_memory), sizeof_fmt(max_memory))))
            ...
            output = "Used memory " + sizeof_fmt(used_memory) + " of " + sizeof_fmt(
                max_memory) + "(%0.2f%%)" % percent_used_memory
        ...
        if tree_xml is not None:
            ...
            if args.connector is None:
                ...
                for connector in tree_xml.findall('./connector'):
                    ...
                    if args.verbosity > 0: print(f"Find {connector_name} connector")
                ...
                if args.verbosity > 0: print(f"Finding {args.connector} connector")
                for connector in tree_xml.findall('./connector'):
                    connector_name = str(connector.get('name'))
